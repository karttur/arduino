 ...
 *
 * RS485/MODBUS:
 * - rs485 communication with temp+sm sensor (ModbusMaster)
 *
...
void full_print_rs485_modbus_temp_sm_ec(float temperature, float sm){
...
void print_rs485_modbus_temp_sm_ec(float temperature, float sm){
...
void rs485_modbus_temp_sm_ec(bool fullprint=0, bool valueprint=1){
  ...
  if (fullprint){
    full_print_rs485_modbus_temp_sm_ec(temperature, sm);
  }
  else if (valueprint) {
    print_rs485_modbus_temp_sm_ec(temperature, sm);
  }
