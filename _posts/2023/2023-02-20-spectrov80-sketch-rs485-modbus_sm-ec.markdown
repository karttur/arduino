---
layout: post
title: rs485/MODBUS temp/SM/EC
categories: sketch
version: v0.80
excerpt: "Arduino sketch for rs485/MODIS temperature, soilmoisture, electric conductivity probe for Spectrolum v080"
tags:
  - spectrolum
  - v080
  - skecth
...
date: '2022-02-20 11:27'
modified: '2022-02-20 T11:27'
comments: true
share: true
---
 ...
 *
 * RS485/MODBUS:
 * - rs485 communication with temp+sm+ec sensor (ModbusMaster)
 *
...
void full_print_rs485_modbus_temp_sm_ec(float temperature, float sm, int ec, int salinity, int tds, float epsilon){

  Serial.print("rs485/modbus temp-sm-ec-sal-tds-e:");
  ...
  Serial.print(" mg/kg\t tds= ");
  Serial.print(tds);
  Serial.print(" mg/kg\t epsilon= ");
  Serial.print(epsilon,1);
  Serial.print(" (unitless)\n");

}

void print_rs485_modbus_temp_sm_ec(float temperature, float sm, int ec, int salinity, int tds, float epsilon){

  Serial.print("rs485/modbus temp-sm-ec-salinity-tds-epsilon:");
  Serial.print(temperature,1);
  Serial.print(',');
  Serial.print(sm,1);
  Serial.print(',');
...
void rs485_modbus_temp_sm_ec(bool fullprint=0, bool valueprint=1){
  ...
  // 0x0000 = temperature (Celsius * 100)
  delay (50);
  resultMain = node.readInputRegisters(0x0000, 1);
  if (resultMain == node.ku8MBSuccess)
  {
    temperature = float(node.getResponseBuffer(0x00))/100.0;
  }

  // 0x0001 = soil moisture (vol/vol)
  delay (50);
  resultMain = node.readInputRegisters(0x0001, 1);
  if (resultMain == node.ku8MBSuccess)
  {
    sm = float(node.getResponseBuffer(0x00))/100.0;
  }

  // 0x0002 = electrical conductivity (mg/L)
  delay (50);
  ...
  // 0x0005 = Epsilon (Dielectric constant)
  resultMain = node.readInputRegisters(0x0005, 1);
  if (resultMain == node.ku8MBSuccess)
  {
    epsilon = float(node.getResponseBuffer(0x00))/100;
  }

  if (fullprint){
    full_print_rs485_modbus_temp_sm_ec(temperature, sm, ec, salinity, tds, epsilon);
  }
  else if (valueprint) {
    print_rs485_modbus_temp_sm_ec(temperature, sm, ec, salinity, tds, epsilon);
  }
...
void loop()
{
  ...
  rs485_modbus_temp_sm_ec(fullprint,valueprint);
