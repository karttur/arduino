 ...
 *
 * RS485/MODBUS:
 * - rs485 communication with temp+sm+ec sensor
 *
...
void full_print_rs485_modbus_temp_sm_ec(int temperature, int sm, int ec, int salinity, int tds, int epsilon){
...
void print_rs485_modbus_temp_sm_ec(int temperature, int sm, int ec, int salinity, int tds, int epsilon){
...
void rs485_modbus_temp_sm_ec(bool fullprint=0, bool valueprint=1){
  ...
  if (resultMain == node.ku8MBSuccess)
  {
    sm = node.getResponseBuffer(0x00);
  }
  ...
  if (resultMain == node.ku8MBSuccess)
  {
    epsilon = node.getResponseBuffer(0x00);
  }
